...
func set_building(cell: Vector2i, building_type: int) -> void:
	if building_type == BuildingType.VOID:
		#空地不存储
...
func get_building(cell: Vector2i) -> int:
	return layer1_data.get(cell, BuildingType.VOID)


## 获取移动成本（综合地形和建筑）
func get_move_cost(cell: Vector2i) -> float:
	var terrain = get_terrain(cell)
	if terrain == TerrainType.VOID or terrain == TerrainType.MOUNTAIN:
		return INF
