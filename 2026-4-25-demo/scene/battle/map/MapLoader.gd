...
const TERRAIN_TO_ATLAS = {
	...
	MapData.TerrainType.RIVER: Vector2i(0, 0),
	MapData.TerrainType.MOUNTAIN: Vector2i(0, 0),
}

# 建筑类型到 atlas 坐标的映射（需要根据你的 TileSet 配置）
const Building_TO_ATLAS = {
	MapData.BuildingType.FOREST: Vector2i(1, 0),
	MapData.BuildingType.VILLAGE: Vector2i(2, 0)
}
...
func _get_terrain_set(terrain_type: int) -> int:
	match terrain_type:
		...
		MapData.TerrainType.RIVER:
			return 0  # Terrain Set 1: 河流
		MapData.TerrainType.MOUNTAIN:
			return 0  # Terrain Set 2: 山脉
		_:
