...
func try_move(unit: Unit, target_cell: Vector2i, map_data: MapData, unit_manager: UnitManager) -> bool:
	# 防御性检查：参数不能为空
	if unit == null or map_data == null or unit_manager == null:
	...
	if path.size() > 0:
		animate_movement.call_deferred(unit, path)
...
func animate_movement(unit: Unit, path: Array[Vector2i], cell_size: int = 64) -> void:
	# 防御性检查：棋子节点必须有效
	...
	for i in range(1, path.size()):
		var cell := path[i]
		# 格子坐标转像素坐标
		var target_pos := Vector2(cell.x * cell_size, cell.y * cell_size)
