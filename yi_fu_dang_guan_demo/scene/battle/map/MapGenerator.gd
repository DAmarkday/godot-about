...
func _noise_to_terrain(noise_value: float) -> int:
	if noise_value < -0.4:
		return MapData.TerrainType.VOID  # 虚空
	...
	for x in range(-1, 2):
		for y in range(-1, 2):
			...
			if map_data.is_in_bounds(cell):
				...
				if x == 0 and y == 0:
					map_data.set_building(cell, MapData.BuildingType.VILLAGE)
...
func _place_spawn_point(map_data: MapData, center: Vector2i) -> void:
	for x in range(-2, 3):
		for y in range(-2, 3):
			...
			if map_data.is_in_bounds(cell):
				map_data.set_terrain(cell, MapData.TerrainType.LAND)
				map_data.set_building(cell,  MapData.BuildingType.VOID)  # 清除建筑
...
##   map_width:   地图宽度（格子数）
##   map_height:  地图高度（格子数）
##   seed_value:  随机种子（相同种子生成相同地图）
## 返回：生成好的 MapData 对象
static func generate_battle_map(map_width: int, map_height: int, seed_value: int) -> MapData:
	# 创建生成器实例
	var generator = MapGenerator.new(map_width, map_height, seed_value)
	
	# 生成基础地图
	var map_data = generator.generate_full_map()
	
	# 强制将玩家出生区域（左下角 3×3，x∈[0,2], y∈[height-3, height-1]）设为 LAND
	for x in range(3):
		for y in range(map_height - 3, map_height):
			var cell = Vector2i(x, y)
			map_data.set_terrain(cell, MapData.TerrainType.LAND)
			map_data.set_building(cell, MapData.BuildingType.VOID)  # 清除建筑层
	
	# 强制将敌方出生区域（右上角 3×3，x∈[width-3, width-1], y∈[0,2]）设为 LAND
	...
	# 再次确保连通性
	generator.ensure_connectivity(map_data)
